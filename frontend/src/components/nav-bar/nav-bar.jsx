...
export default function NavBar({ onAuthButtonPress }) {
  ...
  const isAuthenticated = useCallback(() => state.auth.isAuthenticated, [state]);
    ...
    <nav ref={navBarRef} className="flex items-center justify-between flex-wrap bg-teal-500 p-3">
      ...
      <div className={`w-full ${responsive ? "block" : "hidden"} flex-grow lg:flex lg:items-center lg:w-auto absolute lg:relative top-full lg:top-0 bg-teal-500 -ml-3 lg:ml-0 p-3 lg:p-0 pt-0`}>
        <div className="text-sm lg:flex-grow">
          <Link to="/docs" className="block mt-4 mb-4 lg:mb-0 lg:inline-block lg:mt-0 text-teal-200 hover:text-white mr-4">
            Docs
          </Link>
          <Link to="/examples" className="block mt-4 mb-4 lg:mb-0 lg:inline-block lg:mt-0 text-teal-200 hover:text-white mr-4">
            Examples
          </Link>
          <Link to="/blog" className="block mt-4 mb-4 lg:mb-0 lg:inline-block lg:mt-0 text-teal-200 hover:text-white">
            Blog
          </Link>
        </div>
        <div>
          <button onClick={onAuthButtonPress} className="bg-transparent hover:bg-white text-white font-semibold hover:text-teal-500 py-2 px-4 border border-white hover:border-transparent rounded">
